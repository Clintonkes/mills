    ...
    >
      <div className="max-w-[1320px] mx-auto px-6 lg:px-8 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-8">
          {LINKS.map((l) => (
            <Link key={l.h} to={l.h} className="text-lmills-cream/80 hover:text-lmills-sun text-sm font-medium transition" activeProps={{ className: "text-lmills-sun" }}>{l.l}</Link>
          ))}
          <Link to="/book" className="px-5 py-2.5 rounded-full bg-lmills-sun text-lmills-ink font-semibold text-sm hover:bg-lmills-cream transition">Free Quote</Link>
        </nav>
      ...
      {open && (
        ...
        >
          <div className="px-6 py-4 flex flex-col gap-4">
            {LINKS.map((l) => (
              <Link key={l.h} to={l.h} onClick={() => setOpen(false)} className="text-lmills-cream/85 py-2">{l.l}</Link>
            ))}
            <Link to="/book" onClick={() => setOpen(false)} className="text-center px-5 py-3 rounded-full bg-lmills-sun text-lmills-ink font-semibold">Free Quote</Link>
          </div>
        </motion.div>
      )}
    </header>
